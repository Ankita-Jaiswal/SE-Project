...
def MyPnrTestFn():
 #Setting Credentials
 apiCreds = {}
 apiCreds["Key"] = "ankitajaiswal2890@gmail.com"
 apiCreds["Secret"] = "Ankita@123"

 pnr = "2319107478" #Test Pnr
 requestUrlTemplate ="http://railpnrapi.com/api/check_pnr/pnr/"+pnr+"/format/json/pbapikey/" +\
apiCreds["Key"] + "/pbapisign/"
 ...
 print "From Station (Code/Name): " + jsonOut["from_station"]["code"] + "/" + jsonOut["from_station"]["name"]
 print "To Station (Code/Name): " + jsonOut["to_station"]["code"] + "/" +jsonOut["to_station"]["name"]
 print "Reservation Upto (Code/Name): "+ jsonOut["reservation_upto"]["code"]+ "/" + jsonOut["reservation_upto"]["name"]
 print "Boarding Point (Code/Name): " + jsonOut["boarding_point"]["code"] +"/" + jsonOut["boarding_point"]["name"]
